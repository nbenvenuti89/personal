...
APP = 'BLENDED_MVM_MX' # Switch to MX after full Deployment
...
if Test:
    url = {'PROD': 'https://hooks.slack.com/services/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', # personal-Slackbot 
           'DEV' : 'https://hooks.slack.com/services/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', # personal-Slackbot 
...
if APP=='MVM':
    ...
    where date >= date_sub(CURRENT_DATE(), INTERVAL 15 DAY) and version not like '3.%'
    group by 1 order by 1
    '''  , ctx_Aurora)
elif APP=='MX':
    df_global = pd.read_sql('''
    select CONVERT_TZ(TIMESTAMP(date,concat(HR,':00:00')),'America/Los_Angeles','Asia/Hong_Kong') as LOCAL_HOUR, ''' + sql_globalKpis + ''' from nrtreporting.mx_magnum_bi 
    where date >= date_sub(CURRENT_DATE(), INTERVAL 15 DAY) and version like '3.%'
    group by 1 order by 1
    '''  , ctx_Aurora)
else:
    df_global = pd.read_sql('''
    ...
    try:    
        ...
        if rule_based=='False': 
            ...
            if any(check.SINGLE_ALERT == 'True'):
                replacements = check[check.SINGLE_ALERT == 'True'][['TIMESTAMP','REPLACEMENTS']]
                df2 = df1.merge(replacements, how='left', left_on='ds', right_on='TIMESTAMP')
                df2.y = np.where(df2.REPLACEMENTS.isna(), df2.y, df2.REPLACEMENTS)
                df2.drop(['TIMESTAMP','REPLACEMENTS'], axis=1, inplace=True)
            else:
                df2 = df1
            ...
            for i in range(len(real)):
                if real[i] < danger_lower[i]: OTA.append('Lower')
                elif real[i] > danger_upper[i]: OTA.append('Higher')
                else: OTA.append('False')                
            '''          
            #--------------------------------#            
            # Append info in the results table
            #--------------------------------#
...
if Local:
    ...
    for index, row in forecast.iterrows(): # Update table MVMMART.PUBLIC.BI_ALERT_TEST with the results of the alerts
        if row['Rule_Based'] == 'True':
            try:
                cursor.execute("INSERT INTO "+str(Database[Test])+" (KPI, TIMESTAMP, REAL_VALUES, PREDICTIONS, WARNING_LOWER_BOUND, WARNING_UPPER_BOUND, ALERT_LOWER_BOUND, ALERT_UPPER_BOUND, RMSE, RMSE_TEST, IS_WARNING, IS_ALERT, RUN_TIMESTAMP, IS_RULE_BASED, KPI_CODE, APP, SINGLE_ALERT, RANK) "  
                               "VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                               (str(row['KPI']),str(row['Date']),str(row['Real_Values']),None,None,None,str(row['Alert_Lower_Bound']),str(row['Alert_Upper_Bound']),None,None,str(row['Is_Warning']),str(row['Is_Alert']),str(datetime.now(pytz.timezone('Asia/Hong_Kong')).strftime("%Y-%m-%d %H")+':00:00'),str(row['Rule_Based']),str(row['KPI_Code']),str(APP),str(row['OTA']),str(row['Rank'])))
