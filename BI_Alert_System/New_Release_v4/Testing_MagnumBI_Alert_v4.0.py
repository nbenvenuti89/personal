# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 17:56:15 2019
...
# Parameters
Local = True # If local does not send Slack Messages / Save results in DB 
Test = True
...
days = pd.date_range('2019-10-24 07:50:00', periods=96, freq='H').tolist()
for i, current_date in enumerate(days):
    ...
    if Test:
        ...
        select CONVERT_TZ(TIMESTAMP(date,concat(HR,':00:00')),'America/Los_Angeles','Asia/Hong_Kong') as LOCAL_HOUR, ''' + sql_globalKpis + ''' from nrtreporting.mvm_bi_alert_new 
        ...
        history = pd.read_sql('''
        select *,
        row_number() over (partition by TIMESTAMP, KPI order by TIMESTAMP) as rank from mvmmart.public.BI_ALERT_TEST
        where TIMESTAMP >= dateadd('hour', -333, (select max(TIMESTAMP) from mvmmart.public.BI_ALERT_TEST))
        ...
        history = pd.read_sql('''
        select *,
        row_number() over (partition by RUN_TIMESTAMP, KPI order by TIMESTAMP) as rank from mvmmart.public.BI_ALERT_TEST
        where TIMESTAMP between dateadd('hour', -333, (select max(TIMESTAMP) from mvmmart.public.BI_ALERT_TEST))
        and dateadd('hour', -1, (select max(TIMESTAMP) from mvmmart.public.BI_ALERT_TEST)) 
        order by TIMESTAMP, KPI, RANK
        '''  , ctx_Asia)
        ctx_Asia.close()
    ...
    url = {'PROD': 'https://hooks.slack.com/services/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', # bi-alrt
           'DEV' : 'https://hooks.slack.com/services/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'} # personal  
    ...
    try:    
        ...
        if rule_based=='False': 
            ...
            real = np.array(df2.y[len(df2)-num_hours:len(df2)])
            ...
            #--------------------------------#
            for nrows in range(num_hours):
                forecast = forecast.append(pd.DataFrame({'KPI_Code': [kpiCode], 'KPI': [kpiName], 'Date': [pd.to_datetime(str(start_date[nrows]))], 'Real_Values':[real[nrows]], 'Predictions':None, 'Warning_Lower_Bound' : None,'Warning_Upper_Bound' : None,'Alert_Lower_Bound' : [lower_bound],'Alert_Upper_Bound' : [upper_bound], 'RMSE':None, 'RMSE_Test':None, 'Is_Warning':['False'], 'Is_Alert':[is_danger], 'Rule_Based':[rule_based]}))
                if nrows == 0:
    ...
    if Local:
        ...
        for index, row in forecast.iterrows(): # Update table MVMMART.PUBLIC.BI_ALERT_TEST with the results of the alerts
            ...
            try:
                cursor.execute("INSERT INTO MVMMART.PUBLIC.BI_ALERT_TEST (KPI, TIMESTAMP, REAL_VALUES, PREDICTIONS, WARNING_LOWER_BOUND, WARNING_UPPER_BOUND, ALERT_LOWER_BOUND, ALERT_UPPER_BOUND, RMSE, RMSE_TEST, IS_WARNING, IS_ALERT, RUN_TIMESTAMP, IS_RULE_BASED, KPI_CODE) "  
    ...
    if Test:    
        if rule_based == 'True':
            ...
            fig = Functions.plotAlertManagament(df1, hours_to_plot, kpiName, AM_upper_bound, AM_lower_bound, floor)
        fig
        fig.savefig('C:\\Users\\niccolo\\Desktop\\BI_Alert_System\\New_Release_v4\\Test_Results\\'+str(i)+'-'+str(is_danger)+' - Alert of '+str(kpiCode)+' in '+str(current_date.strftime("%d-%b-%Y_%H"))+'00.png')
